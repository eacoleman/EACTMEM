...
void BackgroundEstimator::readHistograms()
{
   ...
   for(unsigned int i = 0; i < histNames.size(); i++)
   {
      ...
      {
         if(leptonName == "electron")
            Histograms::monteCarloHistograms[name] = (TH1D*) mcList->FindObject((prefix + "_" + name + "_electron").c_str());
         else
            Histograms::monteCarloHistograms[name] = (TH1D*) mcList->FindObject((prefix + "_" + name + "_muon").c_str());
      }
      else
   ...
   if(leptonName == "electron")
      ...
      Histograms::dataHistogram = (TH1D*) dataList->FindObject((prefix + "_SingleEl_Data").c_str());
   else
      //Histograms::dataHistogram = (TH1D*) dataList->FindObject((prefix + "_Data (mu)").c_str());
      Histograms::dataHistogram = (TH1D*) dataList->FindObject((prefix + "_SingleMu_Data").c_str());
   if (debug)
...
void BackgroundEstimator::writeHistograms()
{
   ...
   cout << "\tBackgroundEstimator::writeHistograms Scaling input histograms...";
   ...
   cout << "\tBackgroundEstimator::writeHistograms Writing output histograms...";
   TFile rootOutFile;
