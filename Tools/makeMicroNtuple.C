...
//#include "/uscms_data/d2/aperloff/CMSSW_4_2_8/src/TAMUWW/MEPATNtuple/interface/METree.hh"
//#include "/uscms_data/d2/aperloff/CMSSW_4_2_8/src/TAMUWW/MEPATNtuple/interface/MicroNtuple.hh"
//#include "/uscms_data/d2/aperloff/CMSSW_4_2_8/src/TAMUWW/MEPATNtuple/interface/EventNtuple.hh"
...
void createAllMicroNtuples(){
  ...
  string basePath2="/uscms_data/d2/aperloff/Spring12ME7TeV/MEResults/";

  //Input files for 2-jet bin
  string inputPath=basePath+"rootOutput_oldStructure/";
  unsigned nJet = 2;
  ...
  //Output files
  string outputPath=basePath2+"microNtuples_oldStructure/";
...
//   listOfMicroNtuples.push_back(untagnonWMyStr);
  ...
  listOfMicroNtuples.push_back(MyStr("WJets_matchingup*","microWJets_matchingup_EPDv01",false,false,false));
  listOfMicroNtuples.push_back(MyStr("WJets_matchingdown*","microWJets_matchingdown_EPDv01",false,false,false));
  listOfMicroNtuples.push_back(MyStr("WJets_scaleup*","microWJets_scaleup_EPDv01",false,false,false));
  listOfMicroNtuples.push_back(MyStr("WJets_scaledown*","microWJets_scaledown_EPDv01",false,false,false));
...
{
  cout << "\tcreateAFlatMicroNtuple::Begin" << endl;
  double in_Mjj, in_DRlj1, in_DRlj2, in_epdPretagWWandWZ;
  int in_leptonCat;
  int in_run, in_event;
  double in_eventProb[29];

  vector <TLorentzVector> lLV, jLV, METLV;
  double out_Mjj, out_WmT, out_lEta, out_lPt, out_DRlj1, out_DRlj2, out_j1pT, out_j2pT, out_j1Eta, out_j2Eta, out_MET, out_DEtajj, out_Ptjj, out_EPD, out_DPhiMETj1;
  int out_leptonCat, out_run, out_event;
  vector<double> out_eventProb(29,0.0);
  //double out_eventProb0;
  //double out_eventProb1;

  cout << "\tcreateAFlatMicroNtuple::Get input trees and ntuples...";
  TFile* infile = new TFile(inFileName,"READ");
  TTree* InTree = (TTree*)infile->Get(treeName);
  InTree->Show(0);
  InTree->SetBranchAddress("run",&in_run);
  InTree->SetBranchAddress("event",&in_event);
  InTree->SetBranchAddress("lLV",&lLV);
  InTree->SetBranchAddress("jLV",&jLV);
  InTree->SetBranchAddress("METLV",&METLV);
  InTree->SetBranchAddress("leptonCat",&in_leptonCat);
  InTree->SetBranchAddress("Mjj",&in_Mjj);
  InTree->SetBranchAddress("DRlj1",&in_DRlj1);
  InTree->SetBranchAddress("DRlj2",&in_DRlj2);
  InTree->SetBranchAddress("epdPretagWWandWZ",&in_epdPretagWWandWZ);
  InTree->SetBranchAddress("eventProb[29]",&in_eventProb);
  ...
  cout << "\tcreateAFlatMicroNtuple::Set output file, tree, and branches...";
  ...
  OutTree->Branch("DPhiMETj1",&out_DPhiMETj1,"DPhiMETj1/D");
  OutTree->Branch("run",&out_run,"run/I");
  OutTree->Branch("event",&out_event,"event/I");
  char name[1024];
  char name2[1024];
  for(int k=0; k<29; k++) {
     sprintf(name,"eventProb%d",k);
     sprintf(name2,"eventProb%d/D",k);
     OutTree->Branch(name,&out_eventProb[k],name2);
  }
  //OutTree->Branch("eventProb0",&out_eventProb0,"eventProb0/D");
  //OutTree->Branch("eventProb1",&out_eventProb1,"eventProb1/D");
  cout << "DONE" << endl;
  ...
  cout << "\tcreateAFlatMicroNtuple::Fill output branches...";
  for (unsigned i=0; i<nEntries; i++) {
    ...
    out_DPhiMETj1=abs(abs(abs(METLV[0].Phi()-jLV[0].Phi())-TMath::Pi())-TMath::Pi());

    //out_EPD=log(InMNT->epdPretagWWandWZ);
    out_EPD=log(in_epdPretagWWandWZ);

    //cout << "Mjj=" << out_Mjj << endl;
    //out_Mjj=in_Mjj;

    for(int j=0; j<29; j++) {
       //out_eventProb[j]=InMNT->eventProb[j];
       out_eventProb[j]=in_eventProb[j];
    }
    //out_run=InEvtTree->run;
    //out_event=InEvtTree->event;
    out_run=in_run;
    out_event=in_event;
  ...
  cout << "\tcreateAFlatMicroNtuple::Write output file...";
  outfile->Write();
  outfile->Close();
  cout << "DONE" << endl;
  ...
  delete InTree;
  //delete InEvtTree;
  cout << "\tcreateAFlatMicroNtuple::End" << endl;
}
...
{
  cout << "createMultipleFlatMicroNtuples::Begin" << endl;
  const int NFiles=15;
  //TString dirName = "/uscms_data/d3/ilyao/Summer11/428Full/MEResults/microNtuples/";
  TString dirName = "/uscms_data/d2/aperloff/Spring12ME7TeV/MEResults/microNtuples_oldStructure/";
...
//   fileName[5]="microWpJ_FullCutsV01JESm1s.root";
  ...
  cout << "createMultipleFlatMicroNtuples::Set filenames...";

  fileName[0]="microQCD_El_EPDv01.root";
  fileName[1]="microQCD_Mu_EPDv01.root";
  fileName[2]="microSTopS_T_EPDv01.root";
  fileName[3]="microSTopS_Tbar_EPDv01.root";
  fileName[4]="microSTopTW_T_EPDv01.root";
  fileName[5]="microSTopTW_Tbar_EPDv01.root";
  fileName[6]="microSTopT_T_EPDv01.root";
  fileName[7]="microSTopT_Tbar_EPDv01.root";
  fileName[8]="microSingleEl_Data_EPDv01.root";
  fileName[9]="microSingleMu_Data_EPDv01.root";
  fileName[10]="microTTbar_EPDv01.root";
  fileName[11]="microWJets_EPDv01.root";
  fileName[12]="microWW_EPDv01.root";
  fileName[13]="microWZ_EPDv01.root";
  fileName[14]="microZJets_EPDv01.root";
  cout << "DONE" << endl;
  ...
  cout << "createMultipleFlatMicroNtuples::End" << endl;
}
