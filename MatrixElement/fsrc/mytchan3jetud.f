      SUBROUTINE MYTCHAN3JETUD(P1,MTOP,MW,ANS)
C  
...
C              IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MMYTCHAN3JETUD(P1 ,MTOP, MW, NHEL(1,IHEL))            
C                 IF (T .GT. 0D0) THEN
C                    PRINT *, "HEL: ", IHEL, T
C                 ENDIF
                 ANS(1)=ANS(1)+T
...
C          CALL EXIT(0)
      ...
      REAL*8 FUNCTION MMYTCHAN3JETUD(P,MTOP, MW,NHEL)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C      JAMP(   2) = -AMP(   1)
      ...
      MMYTCHAN3JETUD = AMP(1) * DCONJG(AMP(1)) * 12
C      IF (MMYTCHAN3JETUD .GT. 0D0) THEN
C      PRINT *, "AMP: ", AMP
C      ENDIF
C      CALL EXIT(1)
C
