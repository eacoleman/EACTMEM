      SUBROUTINE MYWBB3JET(P1,MW,ANS)
C  
...
C              IF (GOODHEL(IHEL,IPROC) .OR. NTRY .LT. 2) THEN
                 T=MMYWBB3JET(P1 ,MW,NHEL(1,IHEL))            
                 ANS(1)=ANS(1)+T
C                 IF (T . GT. 0D0) THEN
...
C          CALL EXIT(1)
      ...
      REAL*8 FUNCTION MMYWBB3JET(P,MW,NHEL)
C  
C Generated by MadGraph II                                              
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C      PRINT *,"W: ", W(6,1), W(6,2), W(6,3), W(6,4), W(6,5), W(6,6),
C     .W(6,7)
      CALL JIOXXX(W(1,3   ),W(1,4   ),GWF ,WMASS   ,WWIDTH  ,W(1,8   ))    
C      PRINT *,"W8: ", W(1,8), W(1,9), W(1,10), W(1,11)
...
C      PRINT *, "AMP: ", AMP
      ...
      DO I = 1, NCOLOR
          ...
          ENDDO
C          PRINT *, "ZTEMP, JAMP, DENOM ", ZTEMP, JAMP(I), DENOM(I)
          MMYWBB3JET =MMYWBB3JET+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
C      IF (MMYWBB3JET .GT. 0D0) THEN       
C      DO I = 1, NCOLOR
C          ZTEMP = (0.D0,0.D0)
C          DO J = 1, NCOLOR
