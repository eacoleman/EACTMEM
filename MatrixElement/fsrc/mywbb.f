...
C      DO IHEL=1,NCOMB
      DO IHEL=25,28
        T=MATRIX_WBB(P1,MW,NHEL(1,IHEL))		
        IF (T .GT. 0) THEN 
        ANS(1)=ANS(1)+T
...
C      CALL EXIT(1)
      ...
      REAL*8 FUNCTION MATRIX_WBB(P,MW,NHEL)
C  
C Generated by MadGraph II Version 3.0. Updated 02/19/04                
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C      wwidth  = 2.04759d0   ! PDG best value. Overidden if lwdecay=.true. 
      twidth  = 1.6d0 
...
C      PRINT *, "W(1-6) ", W(1,1), W(1,2), W(1,3), W(1,4), W(1,5), W(1,6)
C      PRINT *, "W(7-10) ", W(1,7), W(1,8), W(1,9), W(1,10)
      PRINT *, "W(8) ", W(1,8), W(2,8), W(3,8), W(4,8), W(5,8), W(6,8)
C      CALL EXIT(1)
C      JAMP(   1) = -AMP(   1)-AMP(   2)
C      MATRIX_WBB = JAMP(1)*DCONJG(JAMP(1))*2.0d0
      PRINT *, "AMP(1), AMP(2) ", AMP(1), AMP(2)
C      CALL EXIT(0)
      JAMP(   1) = -AMP(   1)-AMP(   2)
      PRINT *, "JAMP(1) ", JAMP(1)
      MATRIX_WBB = 0.D0 
      DO I = 1, NCOLOR
          ...
          ENDDO
          MATRIX_WBB =MATRIX_WBB+ZTEMP*DCONJG(JAMP(I))/DENOM(I)   
      ENDDO
