      SUBROUTINE MYSCHAN(P1,MTOP,MW,ANS)
C  
...
C      DO IHEL=1,NCOMB
      DO IHEL=25,28
        T=MMYSCHAN(P1 ,MTOP,MW,NHEL(1,IHEL))		
        IF (T .GT. 0) THEN 
...
C      PRINT *, "DBLE(IDEN(1)) ", DBLE(IDEN(1))
C      PRINT *," ME2 for MTOP",ANS(1), MTOP
C      CALL EXIT(1)
      END

      REAL*8 FUNCTION MMYSCHAN(P,MTOP,MW,NHEL)
C  
C Generated by MadGraph II Version 3.0. Updated 02/19/04                
C RETURNS AMPLITUDE SQUARED SUMMED/AVG OVER COLORS
...
C       PRINT *, "Mass, hel, dir: ", ZERO, NHEL(1), +1
C      PRINT *, "W(1) ", W(1,1), W(1,2), W(1,3), W(1,4), W(1,5), W(1,6)
...
C      PRINT *, "W(9) ", W(1,9), W(2,9), W(3,9), W(4,9), W(5,9), W(6,9)
      CALL IOVXXX(W(1,6   ),W(1,9   ),W(1,7   ),GWF ,AMP(1   ))            
C      PRINT *, "AMP(1) ", AMP(1)
      JAMP(   1) = -AMP(   1)
      MMYSCHAN = JAMP(1)*DCONJG(JAMP(1))*9.0d0
