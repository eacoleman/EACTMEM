...
//      assert ( muonHandle.isValid() );
	 ...
	 d0 = elIter->userFloat("d0_TransientReco");
	 d0err = elIter->userFloat("d0err_TransientReco");
	 d0sig= elIter->userFloat("d0sig_TransientReco");
	 d0T = elIter->userFloat("d0T_TransientReco");
	 d0Terr = elIter->userFloat("d0Terr_TransientReco");
	 d0Tsig= elIter->userFloat("d0Tsig_TransientReco");
	 //	 cout << "d0/d0err=" << d0sig << endl;
	 ...
	 if ( (let>30.0)&&
	      ...
	      //(d0Tsig<3.0)&&
	      (d0sig<3.0)&&
	      ((elIter->electronID("eidRobustTight"))>0.001) 
	      ) {
	   //&&(abs(elIter->dB())<d0_Max)&&((elIter->vertexChi2())<lvtxChi2_Max)
	   lcnt_kin++;
	   elcnt_kin++;
	   ///Isolation Cut
	   lRelIso=(elIter->dr04TkSumPt()+elIter->dr04EcalRecHitSumEt()+elIter->dr04HcalTowerSumEt())/let;
	   if ( lRelIso<0.1 ) {
	     lEta=leta;
	     lPhi=elIter->phi();
