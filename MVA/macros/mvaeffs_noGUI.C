...
class StatDialogMVAEffs {  
   ...
   StatDialogMVAEffs(Float_t ns, Float_t nb);
...
{
   fNSignal = 1000;//fSigInput->GetNumber();
}

void StatDialogMVAEffs::SetNBackground() 
{
   fNBackground = 1000;//fBkgInput->GetNumber();
}
...
StatDialogMVAEffs::StatDialogMVAEffs(Float_t ns, Float_t nb) :
   fNSignal(ns),
   fNBackground(nb),
   fFormula(""),
   fInfoList(0),
...
void StatDialogMVAEffs::DrawHistograms() 
{
   ...
   while ( (info = (MethodInfo*)next()) ) {
      ...
      if (Save_Images) {
         TMVAGlob::imgconv( c, Form("plots/mvaeffs_%s_fNSig%1.0f_fNBkg%1.0f", info->methodTitle.Data(),fNSignal,fNBackground) ); 
      }
      countCanvas++;
   }
}
...
void mvaeffs_noGUI( Float_t fNSignal, Float_t fNBackground, TString fin = "TMVA.root", 
              Bool_t useTMVAStyle = kTRUE, TString formula="S/sqrt(S+B)" )
{
   TMVAGlob::Initialize( useTMVAStyle );
   TFile* file = TMVAGlob::OpenFile( fin );

   //StatDialogMVAEffs* gGui = new StatDialogMVAEffs(gClient->GetRoot(), fNSignal, fNBackground);
   StatDialogMVAEffs* gGui = new StatDialogMVAEffs(fNSignal, fNBackground);
