...
//                bh_xsfit_expect
//    17 Feb 2008 Minor bugfix in bh_xsfit and bh_xsfit_expect -- make sure not to drop the
...
#define MCLIMIT_CSM_VERSION_NUMBER 3.48
#define MCLIMIT_CSM_VERSION_DATE "Feb 26, 2009"
...
void mclimit_csm::run_pseudoexperiments()
{
  ...
  //cout << "size=" << null_hypothesis_pe->channame.size() << endl;
  TH1** pdarray = new TH1*[null_hypothesis_pe->channame.size()];///<<<---FAILS
...
void csm_channel_model::candcheck(TH1 *dh)
{
  ...
  Double_t bsum = 0;
  Double_t dsum = 0;
  const int nbinsx = histotemplate[0]->GetNbinsX();
  const int nbinsy = histotemplate[0]->GetNbinsY();
  ...
  //  cout << "ERROR: THE REST OF THE FUNCTION HAS BEEN COMMENTED OUT" << endl;
  ////
  //Double_t hcb[nbinsx][nbinsy];
  //Double_t hcs[nbinsx][nbinsy];
  vector< vector<double> > hcb(nbinsx, vector<double> (nbinsy,0) );
  vector< vector<double> > hcs(nbinsx, vector<double> (nbinsy,0) );
	  ...
	  for (ibiny=0;ibiny<nbinsy;ibiny++)
	    {
	      if (scaleflag[ic])
		{
		  hcs.at(ibinx).at(ibiny) += sft_varied[ic]*histotemplate_varied[ic]->GetBinContent(ibinx+1,ibiny+1);
		  if (hcs.at(ibinx).at(ibiny) < 0) 
		    {
		      cout << "Negative signal expectation (" << ibinx+1 << "," << ibiny+1 << "):" << hcs.at(ibinx).at(ibiny) << endl;
		    }
		}
	      else
		{
		  hcb.at(ibinx).at(ibiny) += sft_varied[ic]*histotemplate_varied[ic]->GetBinContent(ibinx+1,ibiny+1);
		  if (hcb.at(ibinx).at(ibiny) < 0) 
		    {
		      cout << "Negative background expectation (" << ibinx+1 << "," << ibiny+1 << "):" << hcb.at(ibinx).at(ibiny) << endl;
		    }
		}
	    }
	}
  ...
  for (ibinx=0;ibinx<nbinsx;ibinx++)
    {
      for (ibiny=0;ibiny<nbinsy;ibiny++)
	{
	  ssum += hcs.at(ibinx).at(ibiny);
	  bsum += hcb.at(ibinx).at(ibiny);
          Double_t dc = dh->GetBinContent(ibinx+1,ibiny+1);
	  //cout << "*@*" << hcs.at(ibinx).at(ibiny) << " " << hcb.at(ibinx).at(ibiny) << " " << dc << endl;
	  dsum += dc;
	  if (hcs.at(ibinx).at(ibiny)>0 && hcb.at(ibinx).at(ibiny)<=0)
	    {
	      cout << "Null background with expected signal: (" << ibinx+1 << "," << ibiny+1 
                   << ") Cands: " << dc << " Signal: " << hcs.at(ibinx).at(ibiny) << endl;
	    }
	  else
	    {
	      if (dc > 0)
		{
		  Double_t sbratio = hcs.at(ibinx).at(ibiny)/hcb.at(ibinx).at(ibiny);
		  sumsb += dc*sbratio;
		  if (sbratio>0.3)
		    {
		      cout << "High s/b candidate(s): (" << ibinx+1 << "," << ibiny+1 << ") cands: " << dc << " s/b: " << sbratio << 
			" s: " << hcs.at(ibinx).at(ibiny) << " b: " << hcb.at(ibinx).at(ibiny) << endl;
		    }
		}
	    }
	}
...
Double_t csm_channel_model::chisquared1(TH1 *dh)
{
  ...
  vector<int> lpoissflag(nc);                       // reclassify it as no bin error
...
//   Double_t sfgp[nc];  // scale factor needed to approximate Gaussian errors as Poisson
  vector<double> rho1(nc);
  vector<double> rho2(nc);
  vector<int> zlist(nc);
  vector<double> sfgp(nc);
  ...
  for (ibinx=0;ibinx<nbinsx;ibinx++)
    {
	  ...
	  for (ic=0;ic<nc;ic++)
		...
		{ lpoissflag.at(ic) = CSM_NOBINERR; }
	      if (lpoissflag.at(ic) == CSM_GAUSSIAN_BINERR)
		{
		  sfgp.at(ic) = gbc/(gbe*gbe);
		  lpoissflag.at(ic) = CSM_POISSON_BINERR;
		}
	    }
	  ...
	  /* the sum of zero-bin-error contributions, varied by the nuisance parameters */
          ...
          for (ic=0;ic<nc;ic++)
 	    {
	      if (lpoissflag.at(ic) == CSM_NOBINERR)
	        {
		  if (nbinsy == 1)
		    { gbc = histotemplate_varied[ic]->GetBinContent(ibinx+1); }
		  else
          ...
          for (iter=0;iter<CSM_MAXITER;iter++)
	    {
	      ...
	      for (ic=0;ic<nc;ic++)
		      ...
		      if (zlist.at(ic) == 0)
			{
		            ...
		            { if ( (lpoissflag.at(ip1)==CSM_POISSON_BINERR) && ip1 != ic) D += rho1.at(ip1); }
	                  A = 1.0 + sfgp.at(ic)/sft_varied[ic];
		          if (nbinsy == 1)
			    { gbc = sfgp.at(ic)*max(0,histotemplate_varied[ic]->GetBinContent(ibinx+1)); }
		          else
			    { gbc = sfgp.at(ic)*max(0,histotemplate_varied[ic]->GetBinContent(ibinx+1,ibiny+1)); }
		          B = A*D - dtb - gbc;
		          C = -gbc*D;
	                  rho2.at(ic) = (-B + sqrt(B*B - 4.0*A*C))/(2.0*A);
			  //cout << "ABC: " << A << " " << B << " " << C << endl;
			}
			...
			{                    // if we have zero MC -- the "strongest" one among all the contributions
			  rho2.at(ic) = 0;     // with zero MC prediction
			}
		    }
	        }
	      iprec = 0;
	      ...
	      for (ic=0;ic<nc;ic++)
	        {
	          if (lpoissflag.at(ic) == CSM_POISSON_BINERR)
	            {
	              if (fabs(rho1.at(ic)) < PREC1)
		        { 
		          if (fabs(rho2.at(ic)-rho1.at(ic)) > PREC1)
		            { 
                              iprec = 1;
		              break;
		            }
		        ...
		        {
		          if ( fabs((rho2.at(ic)-rho1.at(ic))/rho1.at(ic))>PREC1 )
		            {
		              iprec = 1;
		              break;
		            }
	  ...
	  */
	  ...
	  for (ic=0;ic<nc;ic++)
	    {
	      if ( (lpoissflag.at(ic) == CSM_POISSON_BINERR) && sft_varied[ic] > 0)
		{
		  if (nbinsy == 1)
		    { nsubs = sfgp.at(ic)*histotemplate_varied[ic]->GetBinContent(ibinx+1); }
		  else
		    { nsubs = sfgp.at(ic)*histotemplate_varied[ic]->GetBinContent(ibinx+1,ibiny+1); }

#ifdef DEBUGPRINTC2
		  Double_t c2cont = ( rho2.at(ic)*sfgp.at(ic)/sft_varied[ic] - nsubs);
		  if (nsubs > 0 && rho2.at(ic) > 0)
		    c2cont -= ((Double_t) nsubs)*log(rho2.at(ic)*sfgp.at(ic)/(sft_varied[ic]*((Double_t) nsubs)));
		  if (c2cont > 0.0)
		  //  if (ibinx == 9 && ibiny == 4)
		    {
		    cout << "in chi2calc: " << ibinx << " " << ibiny << " " << ic << " " << 
		      rho2.at(ic) << " " << sft_varied[ic] << " " << nsubs << " " << c2cont;
		    if (c2cont>0.01) 
		      {
			cout << "*" << endl;
		      }
...
#endif
		  chi2a += (rho2.at(ic)*sfgp.at(ic)/sft_varied[ic] - nsubs);
		  if (nsubs > 0 && rho2.at(ic) > 0)
		    chi2a -= ((Double_t) nsubs)*log(rho2.at(ic)*sfgp.at(ic)/(sft_varied[ic]*((Double_t) nsubs)));
		}
	    }
	      ...
	      for (ic=0;ic<nc;ic++)
		{
		  if ( (lpoissflag.at(ic) == CSM_POISSON_BINERR) && sft_varied[ic] > 0)
			...
			{ nsubs = sfgp.at(ic)*histotemplate_varied[ic]->GetBinContent(ibinx+1,ibiny+1); }
		      ...
		      /*
			cout << "in chi2calc: " << ibinx << " " << ibiny << " " << ic << " " << 
			rho1.at(ic) << " " << sft_varied[ic] << " " << nsubs << " " sfgp.at(ic) << endl;
		      */
		      chi2b += (rho1.at(ic)*sfgp.at(ic)/sft_varied[ic] - nsubs);
		      if (nsubs > 0 && rho1.at(ic) > 0)
			chi2b -= ((Double_t) nsubs)*log(rho1.at(ic)*sfgp.at(ic)/(sft_varied[ic]*((Double_t) nsubs)));
		    }
		}
	    }
          else
...
//#define DEBUGPVMC
void VSort(Int_t n1, vector<double>& a, vector<int>& index, Bool_t down)
{
   //  Sort the n1 elements of the vector a.
	      ...
	      Double_t par1, Double_t par2, Double_t parn)
{
  ...
  Double_t total;
  Double_t wta,wtb;
  vector<double> yd(nb3);
  vector<double> id(nb3);
  vector<double> xd(nb3);
  vector<double> xdis(nb3);
  vector<double> ydis(nb3);
  vector<double> ydi(nb + 1);
  vector<int> idx(nb3);
  Int_t ifirst;
  ...
  //// redefine the TMath::Sort function for the vector
  VSort(j,yd,idx,0);
...
void csm_pvmcV(Int_t nb, vector<double>& dist1, vector<double>& dist2, vector<double>& dist3, vector<double>& distn,
	      Double_t par1, Double_t par2, Double_t parn)
{
  Int_t nb3 = nb*3 + 3;
  Int_t i,j,k,k1,k2;
...
void csm_pvmc2d(Int_t nx,Int_t ny, Double_t *xydist1, 
                ...
                Double_t par1, Double_t par2, Double_t parn)
{
  vector<double> ydist1(ny),ydist2(ny),ydist3(ny),ydistn(ny);
  vector<double> xtemp1(nx),xtemp2(nx),xtemp3(nx),xtempn(nx);
  vector<double> alpha1(ny*ny),alpha2(ny*ny),alpha3(ny*ny);
  Int_t i,j,k;
...
//   csm_yproj(nx,ny,xydist3,ydist3);
  csm_yprojV(nx,ny,xydist1,ydist1);
  csm_yprojV(nx,ny,xydist2,ydist2);
  csm_yprojV(nx,ny,xydist3,ydist3);
  ...
  csm_pvmcV(ny,ydist1,ydist2,ydist3,ydistn,par1,par2,parn);
  ...
  csm_ycontV(ny,ydist1,ydist2,ydist3,ydistn,alpha1,alpha2,alpha3);
  ...
  for (i=0;i<ny;i++)  // loop over resulting bins
      ...
      for (j=0;j<ny;j++) // loop over contributing bins
	{
      ...
      csm_pvmcV(nx,xtemp1,xtemp2,xtemp3,xtempn,par1,par2,parn);
...
void csm_yprojV(Int_t nx, Int_t ny, Double_t *xydist, vector<double>& ydist)
{
  Int_t i,j;
  Double_t total;
...
//   csm_acnvec2V(y,ny+1);
//   csm_ycontauxV(ny,y,yn,alpha3);
// }

void csm_ycontV(Int_t ny, vector<double>& ydist1, vector<double>& ydist2,
               vector<double>& ydist3, vector<double>& ydistn,
               vector<double>& alpha1, vector<double>& alpha2, vector<double>& alpha3)
{
  vector<double> y(ny+1);
  vector<double> yn(ny+1);
  Int_t i;
...
void csm_ycontauxV(Int_t ny, vector<double>& y, vector<double>& yn,
                   vector<double>& alpha)
{
  Int_t ny2;
  Int_t i,j;
...
void csm_acnvec2V(vector<double>& vec, Int_t n)
{
  Int_t i;
  Double_t tot;
  for (i=1;i<n;i++)
